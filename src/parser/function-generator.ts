...
function countTokens(expr: string): number {
  try {
    ...
    return 1000; // Should not happen with valid generated code
  }
}
...
function generateNode(
  ...
  if (maxDepth === 0 || targetTokens <= 1) {
    ...
    if (r < 0.4) return 'x';
    if (r < 0.6) return randomChoice(CONSTANTS);
    return randomInt(-10, 10).toString();
  }

  const choice = Math.random();

  // binary operation
  if (choice < 0.5 && targetTokens > 2) {
